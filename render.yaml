...
class FavoriteStore(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("user_id", "store_id", name="uq_favorite_user_store"),
    )
...
def listing_query_from_request():
    ...
    if seller:
        query = query.join(User, Listing.seller_id == User.id).outerjoin(
            Store, Store.owner_id == User.id
        ).filter(or_(User.name.ilike(f"%{seller}%"), Store.name.ilike(f"%{seller}%")))
...
@app.route("/")
def home():
    query, q, category, state, shipping, seller, price_min, price_max, sort = listing_query_from_request()
    stores = (
...
def dashboard():
    ...
    for listing in listings:
        listing_stats[listing.id] = {
            "views": ListingView.query.filter_by(listing_id=listing.id).count(),
            "favorites": Favorite.query.filter_by(listing_id=listing.id).count(),
            "inquiries": Inquiry.query.filter_by(listing_id=listing.id).count(),
        }

    total_views = sum(item["views"] for item in listing_stats.values())
    total_favorites = sum(item["favorites"] for item in listing_stats.values())
...
def stores_directory():
    ...
    stores = query.order_by(Store.featured.desc(), Store.created_at.desc()).all()
    return render_template("stores.html", stores=stores, q=q)


@app.route("/store/<slug>/favorite", methods=["POST"])
@login_required
def toggle_store_favorite(slug):
    ...
    if favorite:
        db.session.delete(favorite)
        flash("Store removed from your watchlist.", "success")
    else:
        db.session.add(FavoriteStore(user_id=current_user.id, store_id=store.id))
        flash("Store saved to your watchlist.", "success")
    db.session.commit()
...
def storefront(slug):
    ...
    store_is_favorite = False
    if current_user.is_authenticated:
        store_is_favorite = FavoriteStore.query.filter_by(
            user_id=current_user.id, store_id=store.id
        ).first() is not None
    return render_template(
        "storefront.html",
        store=store,
        listings=listings,
        store_is_favorite=store_is_favorite,
    )
...
@login_required
def delete_listing_photo(photo_id):
    photo = db.get_or_404(Photo, photo_id)
    if photo.listing.seller_id != current_user.id:
        abort(403)
...
def listing_detail(listing_id):
    listing = db.get_or_404(Listing, listing_id)
    db.session.add(ListingView(listing_id=listing.id))
    db.session.commit()
    ...
    related = (
        Listing.query.filter(
            ...
            or_(
                Listing.species.ilike(f"%{listing.species}%"),
    ...
    view_count = ListingView.query.filter_by(listing_id=listing.id).count()
    favorite_count = Favorite.query.filter_by(listing_id=listing.id).count()
